...
qfile=./data/question_wordvector/glove_normalized_300_train.pkl.gz
cfile=./data/choice_wordvector/glove_normalized_1500_train.pkl.gz
...
do
    python evaluateLSTMandMLP.py -model ${model} -ionly ${ionly} -lonly ${lonly} -ifdim ${ifdim} -lfdim ${lfdim} -w ${w} -qf ${qfile} -cf ${cfile} -if ${ifile} -predict_type train
done
